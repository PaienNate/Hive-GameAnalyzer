	...
	export default {
		...
		methods: {
			...
			login() {
			  ...
			  this.$api.login.userlogin(userInfo).then((res) => {
			      if(res.code != 200) {
			        this.$message({
			          message: res.msg,
			          type: 'error'
			...
			sendMail() {
				let useremail = {email:this.ruleFormRepass.email};

				//发送重置密码请求，得到邮箱验证码
				this.$api.login.useremail(useremail).then(res=>{
					//检测不是200时，报错提示
					if(res.code!=200)
					{
						this.$message({
			...
			repasswd() {
				...
				this.$api.login.userchangepass(verifyinfo).then(res => {
				if (res.code != 200) {
					//使用弹窗提示错误信息
					this.$message({
						message: res.msg,

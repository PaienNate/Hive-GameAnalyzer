<template>
	...
	<div id="container-register">
		...
		<div class="register">
			...
			<a @click="gologin()" href="#"><button id="register-link">点击这里去登录页面！</button></a>
		</div>
	</div>
	</div>
</template>
...
export default {
	...
	methods: {
		register() {
			this.$api.login.userreg(this.registerForm).then(res => {
				if (res.code != 200) {
					//使用弹窗提示错误信息
					this.$message({
						message: res.msg,
					...
					this.$router.push('/userlogin')  // 登录成功，跳转到主页
				}
				console.log(res)
			})
		},
		...
		{
			this.$store.commit('menuRouteLoaded', false) // 要求重新加载导航菜单
			this.$router.push('/userlogin')  //  要求登录
